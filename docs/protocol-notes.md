...
9b. **A retransmitted final chunk is answered with `off == 0`, not `off == len`.**
   ...
   **Confirmed on hardware in P17**: the NUCLEO-WB55RG peer played exactly this
...
**Measured on the P17 peer:** `buf_size` **2475** and `buf_count` **4**, which
...
| A7 | Implicit slot erase on first chunk has unbounded duration. **Measured in P17:** 6.6 s from the first chunk to `Erased 0x22000 bytes of image slot` on the NUCLEO-WB55RG, where every flash operation is interleaved with the radio's activity. | Separate, longer `first_chunk_timeout` (default 30 s vs 5 s). The measurement leaves the default alone: a slot four times larger on a slower part could still approach it, and the constant is per-upload configurable. |
...
`mcumgr-client` 0.0.9 exposes serial and UDP, not BLE. P17 therefore retains its
UART state comparison and adds `smpmgr` for BLE state/HCI comparison (ADR-0015).
...
### P17c test-method finding: the UART arm is an oracle, not a third client
...
configuration tried (P17c, 2026-09-09). Recorded because it changes what the
