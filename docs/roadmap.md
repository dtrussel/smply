...
| P18a | **`.github/workflows/osv.yml` has not fired on its schedule.** It has run — the push that added it triggered it, since the workflow is in its own `paths` filter — and that run found a defect worth keeping in mind: the scanner action runs in a Docker container that mounts only the workspace, so a path built from `runner.temp` does not exist inside it, and `continue-on-error` turned a total failure into a green job. Fixed, with a step that fails when no SARIF was produced. What remains unproven is the *cron*, and whether `google/osv-scanner-action` finds anything to say about the SPDX `tools/sbom.py` emits — it has now parsed it, but a clean scan of two pinned dependencies is also what a scan of nothing looks like until one of them has an advisory. | at its first scheduled run |
