...
| The Windows targets | `transports/winrt_ble/`, `examples/winrt_ble_dfu/` | — | **compiled only.** `windows-winrt` builds them and runs `winrt_ble_smoke`, which links the adapter and checks it refuses a bad configuration. No radio, so nothing crosses GATT |
