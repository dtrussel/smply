...
# 3b. Four directories are Windows-only and invisible to clang-tidy and
# cppcheck (transports/winrt_ble/, examples/winrt_ble_dfu/, tests/hil/ and
# transports/serial_port/win32/; see tools/lint.sh). An
...
#
# The decoys are portable files whose names contain "winrt", "hil" or "win32",
# one beside each excluded directory so no prefix can quietly widen. The serial
# one matters most: transports/serial_port/ is otherwise analysed, so a filter
# that excluded the whole adapter, or anything named win32, would be a real
# hole. An exact filter keeps all four and the TU count rises by four; an
# over-broad filter drops
...
printf '// SPDX-License-Identifier: Apache-2.0\n' > "$WORK/transports/serial_port/win32_decoy.cpp"
...
expect_ok "the Windows-only lint exclusions are directories, not substrings" \
    bash -c 'expected=$(tools/sources.sh | grep -E "\.(cpp|cc)$" \
                        | grep -Ev "^tests/interface_flags/|^tests/consumption/" \
                        | grep -Ev "^transports/winrt_ble/|^examples/winrt_ble_dfu/|^tests/hil/|^transports/serial_port/win32/" | wc -l)
...
rm -f "$WORK/transports/common/winrt_decoy.cpp" "$WORK/examples/cli_dfu/winrt_decoy.cpp" \
      "$WORK/tests/support/hil_decoy.cpp" "$WORK/transports/serial_port/win32_decoy.cpp" \
      "$WORK/tidy-stub"
