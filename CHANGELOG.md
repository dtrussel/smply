...
- **Several images of one device in one update**
  ...
  device commits itself runs on trial. The device commits those images when
  smply confirms image 0, and smply waits to see it, so `Completed` means the
  whole set is committed
  ([ADR-0022](docs/decisions/ADR-0022-device-images-commit-after-client.md)).
  New types: `ImageTarget`, `CommitBy` (`Client` or `Device`) and
  `ImageReport`, whose `applied` and `committed` say how far each image got.
  `UpdateReport::images` reports each image. `UpdatePlan::apply_timeout` and
  `apply_poll_interval` bound each of the two waits, in the new states
  `UpdateState::AwaitingDeviceApply` and `AwaitingDeviceCommit`. A read that
  finds the link gone during either wait asks the application to reconnect
  and carries on, since the link may run through the MCU being updated; a
  read that times out is retried. The device contract for a device-committed
  image is `docs/multi-image.md`. Tested against the simulator and the stub;
  nothing multi-image has run on hardware.
- **`smply::dfu_package`** (`support/dfu_package/`, not installed): reads the
  ...
  reports each image's MCUboot dependency TLVs, and refuses a package whose
  images depend on a newer version of another image it carries, compared as
  a default MCUboot compares them. Hand-written, bounded, and fuzzed
  (`fuzz_dfu_package`). `smply::dfu_app::PackageUpdate` turns a package
